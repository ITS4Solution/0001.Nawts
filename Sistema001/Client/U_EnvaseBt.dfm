inherited F_EnvaseBt: TF_EnvaseBt
  Caption = 'Ordem de Produ'#231#227'o - Envase'
  ClientHeight = 617
  ClientWidth = 726
  KeyPreview = True
  OnKeyPress = FormKeyPress
  ExplicitWidth = 744
  ExplicitHeight = 664
  PixelsPerInch = 96
  TextHeight = 13
  inherited dxBevel1: TdxBevel
    Top = 0
    Width = 726
    Height = 544
    Align = alClient
    ExplicitTop = 11
    ExplicitWidth = 668
    ExplicitHeight = 515
  end
  inherited Image1: TImage
    Left = 8
    Top = 1
    ExplicitLeft = 8
    ExplicitTop = 1
  end
  inherited dxBevel2: TdxBevel
    Top = -6
    ExplicitTop = -6
  end
  object Label3: TLabel [3]
    Left = 189
    Top = 39
    Width = 38
    Height = 13
    Caption = 'Produto'
  end
  object Label7: TLabel [4]
    Left = 293
    Top = 87
    Width = 3
    Height = 13
  end
  object Label12: TLabel [5]
    Left = 573
    Top = 114
    Width = 31
    Height = 13
    Caption = 'Status'
  end
  object Label1: TLabel [6]
    Left = 189
    Top = 4
    Width = 58
    Height = 13
    Caption = 'C'#243'd.Envase'
    FocusControl = cxDBSpinEdit1
  end
  object Label2: TLabel [7]
    Left = 319
    Top = 4
    Width = 23
    Height = 13
    Caption = 'Data'
    FocusControl = CC
  end
  object Label4: TLabel [8]
    Left = 463
    Top = 4
    Width = 23
    Height = 13
    Caption = 'Hora'
    FocusControl = cxDBTimeEdit1
  end
  object Label5: TLabel [9]
    Left = 544
    Top = 76
    Width = 21
    Height = 13
    Caption = 'Lote'
    FocusControl = cxDBTextEdit1
  end
  object Label6: TLabel [10]
    Left = 420
    Top = 76
    Width = 20
    Height = 13
    Caption = 'CPA'
    FocusControl = cxDBTextEdit2
  end
  object Label8: TLabel [11]
    Left = 322
    Top = 114
    Width = 46
    Height = 13
    Caption = 'Operador'
    FocusControl = cxDBTextEdit3
  end
  object Label9: TLabel [12]
    Left = 189
    Top = 78
    Width = 64
    Height = 13
    Caption = 'Qtd. Prevista'
    FocusControl = DBEdit1
  end
  object Label10: TLabel [13]
    Left = 189
    Top = 114
    Width = 46
    Height = 13
    Caption = 'Qtd. Real'
    FocusControl = DBEdit2
  end
  object Label13: TLabel [14]
    Left = 322
    Top = 76
    Width = 13
    Height = 13
    Caption = 'UE'
  end
  object Label15: TLabel [15]
    Left = 591
    Top = 4
    Width = 63
    Height = 13
    Caption = 'Qtd. Enviada'
    FocusControl = DBEdit2
  end
  inherited StatusBar1: TStatusBar
    Top = 598
    Width = 726
    ExplicitTop = 598
    ExplicitWidth = 726
  end
  inherited Panel1: TPanel
    Top = 544
    Width = 726
    Height = 54
    TabOrder = 12
    ExplicitTop = 544
    ExplicitWidth = 726
    ExplicitHeight = 54
    inherited btnSalvar: TAdvGlassButton
      Left = 317
      Top = 16
      ExplicitLeft = 317
      ExplicitTop = 16
    end
    inherited btnsair: TAdvGlassButton
      Left = 618
      Top = 13
      ExplicitLeft = 618
      ExplicitTop = 13
    end
    object btnImprimi: TAdvGlassButton
      Left = 423
      Top = 13
      Width = 100
      Height = 32
      BackColor = 6052956
      Caption = 'Imprimir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 5
      Version = '1.3.0.2'
      OnClick = btnImprimiClick
    end
  end
  object cxDBSpinEdit2: TcxDBSpinEdit [18]
    Left = 114
    Top = 211
    DataBinding.DataField = 'custoenvase'
    DataBinding.DataSource = oDS
    TabOrder = 13
    Width = 121
  end
  object cxDBLookupComboBox1: TcxDBLookupComboBox [19]
    Tag = 3
    Left = 189
    Top = 54
    DataBinding.DataField = 'idprodutos'
    DataBinding.DataSource = oDS
    Properties.GridMode = True
    Properties.KeyFieldNames = 'id'
    Properties.ListColumns = <
      item
        FieldName = 'descricao'
      end>
    Properties.ListSource = DSProdutos
    Properties.OnChange = cxDBLookupComboBox1PropertiesChange
    Properties.OnCloseUp = cxDBLookupComboBox1PropertiesCloseUp
    TabOrder = 3
    OnExit = cxDBLookupComboBox1Exit
    Width = 396
  end
  object Panel2: TPanel [20]
    Left = 3
    Top = 316
    Width = 723
    Height = 222
    Anchors = [akLeft, akRight, akBottom]
    Caption = 'Panel2'
    TabOrder = 14
    object cxGrid1: TcxGrid
      Left = 1
      Top = 1
      Width = 721
      Height = 220
      Align = alClient
      Anchors = [akLeft, akRight, akBottom]
      TabOrder = 0
      object cxGrid1DBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.First.Enabled = False
        Navigator.Buttons.PriorPage.Enabled = False
        Navigator.Buttons.Next.Enabled = False
        Navigator.Buttons.NextPage.Enabled = False
        Navigator.Buttons.Last.Enabled = False
        Navigator.Buttons.Append.Enabled = False
        Navigator.Buttons.Edit.Enabled = False
        Navigator.Buttons.Refresh.Enabled = False
        Navigator.Buttons.SaveBookmark.Enabled = False
        Navigator.Buttons.GotoBookmark.Enabled = False
        Navigator.Buttons.Filter.Enabled = False
        OnCustomDrawCell = cxGrid1DBTableView1CustomDrawCell
        DataController.DataModeController.GridMode = True
        DataController.DataSource = dsDetalheEnvase
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsBehavior.FocusCellOnTab = True
        OptionsBehavior.FocusFirstCellOnNewRecord = True
        OptionsBehavior.GoToNextCellOnEnter = True
        OptionsData.Appending = True
        OptionsData.DeletingConfirmation = False
        OptionsView.ColumnAutoWidth = True
        object cxGrid1DBTableView1codproduto: TcxGridDBColumn
          Caption = 'C'#243'd.Insumo'
          DataBinding.FieldName = 'codproduto'
          Width = 309
        end
        object cxGrid1DBTableView1qtd: TcxGridDBColumn
          Caption = 'Qtd. Utilizada'
          DataBinding.FieldName = 'qtd'
          Options.Editing = False
          Width = 129
        end
        object cxGrid1DBTableView1ue: TcxGridDBColumn
          Caption = 'UE'
          DataBinding.FieldName = 'ue'
          Width = 50
        end
        object cxGrid1DBTableView1sEstoque: TcxGridDBColumn
          Caption = 'Qtd. Estoque'
          DataBinding.FieldName = 'sEstoque'
          Width = 173
        end
      end
      object cxGrid1Level1: TcxGridLevel
        GridView = cxGrid1DBTableView1
      end
    end
  end
  object cbStatus: TcxDBComboBox [21]
    Tag = 10
    Left = 573
    Top = 129
    DataBinding.DataField = 'status'
    DataBinding.DataSource = oDS
    Properties.Items.Strings = (
      'Aberto'
      'Produzindo'
      'Finalizada')
    TabOrder = 11
    Width = 148
  end
  object cxDBSpinEdit1: TcxDBSpinEdit [22]
    Left = 189
    Top = 18
    DataBinding.DataField = 'codopenvase'
    DataBinding.DataSource = oDS
    TabOrder = 0
    OnKeyPress = cxDBSpinEdit1KeyPress
    Width = 124
  end
  object CC: TcxDBDateEdit [23]
    Tag = 1
    Left = 319
    Top = 18
    DataBinding.DataField = 'data'
    DataBinding.DataSource = oDS
    TabOrder = 1
    OnKeyPress = CCKeyPress
    Width = 138
  end
  object cxDBTimeEdit1: TcxDBTimeEdit [24]
    Tag = 2
    Left = 463
    Top = 18
    DataBinding.DataField = 'hora'
    DataBinding.DataSource = oDS
    TabOrder = 2
    Width = 122
  end
  object cxDBTextEdit1: TcxDBTextEdit [25]
    Tag = 7
    Left = 544
    Top = 91
    DataBinding.DataField = 'lote'
    DataBinding.DataSource = oDS
    TabOrder = 7
    Width = 177
  end
  object cxDBTextEdit2: TcxDBTextEdit [26]
    Tag = 6
    Left = 420
    Top = 91
    DataBinding.DataField = 'cpa'
    DataBinding.DataSource = oDS
    TabOrder = 6
    Width = 117
  end
  object cxDBTextEdit3: TcxDBTextEdit [27]
    Tag = 9
    Left = 322
    Top = 129
    DataBinding.DataField = 'operador'
    DataBinding.DataSource = oDS
    TabOrder = 9
    Width = 245
  end
  object DBEdit1: TDBEdit [28]
    Tag = 4
    Left = 189
    Top = 92
    Width = 127
    Height = 21
    DataField = 'qtdprevista'
    DataSource = oDS
    TabOrder = 4
    OnChange = DBEdit1Change
    OnExit = DBEdit1Exit
  end
  object DBEdit2: TDBEdit [29]
    Tag = 8
    Left = 189
    Top = 129
    Width = 127
    Height = 21
    DataField = 'qtdreal'
    DataSource = oDS
    TabOrder = 8
  end
  object cxDBComboBox2: TcxDBComboBox [30]
    Tag = 5
    Left = 322
    Top = 91
    DataBinding.DataField = 'ue'
    DataBinding.DataSource = oDS
    Properties.Items.Strings = (
      'UN'
      'Vidro'
      'Pote'
      'Outro')
    TabOrder = 5
    OnContextPopup = cxDBComboBox2ContextPopup
    OnExit = cxDBComboBox2Exit
    Width = 92
  end
  object Panel3: TPanel [31]
    Left = 8
    Top = 156
    Width = 723
    Height = 155
    Align = alCustom
    Anchors = [akLeft, akTop, akRight, akBottom]
    Caption = 'Panel3'
    TabOrder = 15
    object cxGrid2: TcxGrid
      Left = 1
      Top = 1
      Width = 721
      Height = 153
      Align = alClient
      Anchors = [akLeft, akRight, akBottom]
      TabOrder = 0
      object cxGridDBTableView1: TcxGridDBTableView
        Navigator.Buttons.CustomButtons = <>
        Navigator.Buttons.First.Enabled = False
        Navigator.Buttons.PriorPage.Enabled = False
        Navigator.Buttons.Next.Enabled = False
        Navigator.Buttons.NextPage.Enabled = False
        Navigator.Buttons.Last.Enabled = False
        Navigator.Buttons.Append.Enabled = False
        Navigator.Buttons.Edit.Enabled = False
        Navigator.Buttons.Refresh.Enabled = False
        Navigator.Buttons.SaveBookmark.Enabled = False
        Navigator.Buttons.GotoBookmark.Enabled = False
        Navigator.Buttons.Filter.Enabled = False
        DataController.DataModeController.GridMode = True
        DataController.DataSource = dsEstoqueQuimico
        DataController.Summary.DefaultGroupSummaryItems = <>
        DataController.Summary.FooterSummaryItems = <>
        DataController.Summary.SummaryGroups = <>
        OptionsBehavior.FocusCellOnTab = True
        OptionsBehavior.FocusFirstCellOnNewRecord = True
        OptionsBehavior.GoToNextCellOnEnter = True
        OptionsData.Appending = True
        OptionsData.DeletingConfirmation = False
        OptionsView.ColumnAutoWidth = True
        object cxGridDBTableView1sDescricao: TcxGridDBColumn
          Caption = 'Produto'
          DataBinding.FieldName = 'sDescricao'
          Width = 297
        end
        object cxGridDBTableView1percentual: TcxGridDBColumn
          Caption = 'Percentual'
          DataBinding.FieldName = 'percentual'
          Width = 149
        end
        object cxGridDBTableView1qtd: TcxGridDBColumn
          Caption = 'Quantidade'
          DataBinding.FieldName = 'qtd'
          Width = 131
        end
      end
      object cxGridLevel1: TcxGridLevel
        GridView = cxGridDBTableView1
      end
    end
  end
  object recSeq: TPanel [32]
    Left = 179
    Top = 231
    Width = 344
    Height = 103
    Enabled = False
    TabOrder = 16
    Visible = False
    object Label11: TLabel
      AlignWithMargins = True
      Left = 4
      Top = 4
      Width = 336
      Height = 14
      Align = alTop
      Caption = 'Digite o c'#243'digo sequ'#234'ncial do CI '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitWidth = 201
    end
    object edtSeq: TEdit
      AlignWithMargins = True
      Left = 25
      Top = 30
      Width = 288
      Height = 21
      TabOrder = 0
    end
    object AdvGlassButton2: TAdvGlassButton
      Left = 7
      Top = 64
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Cancelar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 1
      Version = '1.3.0.2'
      OnClick = AdvGlassButton2Click
    end
    object AdvGlassButton3: TAdvGlassButton
      Left = 174
      Top = 64
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Enviar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 2
      Version = '1.3.0.2'
      OnClick = AdvGlassButton3Click
    end
    object btnCI: TAdvGlassButton
      Left = 91
      Top = 64
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Gerar CI'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 3
      Version = '1.3.0.2'
      OnClick = btnCIClick
    end
    object AdvGlassButton1: TAdvGlassButton
      Left = 257
      Top = 64
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Enviar sem CI'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 4
      Version = '1.3.0.2'
      OnClick = AdvGlassButton1Click
    end
  end
  object AdvGlassButton4: TAdvGlassButton [33]
    Left = 602
    Top = 45
    Width = 95
    Height = 40
    BackColor = 6052956
    Caption = 'Enviar parte da quantidade'
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ForeColor = clWhite
    GlowColor = 16760205
    InnerBorderColor = clBlack
    OuterBorderColor = clWhite
    ParentFont = False
    ShineColor = clWhite
    TabOrder = 17
    Version = '1.3.0.2'
    OnClick = AdvGlassButton4Click
  end
  object panelQtd: TPanel [34]
    Left = 135
    Top = 153
    Width = 458
    Height = 72
    TabOrder = 18
    Visible = False
    object Label14: TLabel
      Left = 20
      Top = 16
      Width = 124
      Height = 13
      Caption = 'Quantidade a ser enviada'
      FocusControl = DBEdit3
    end
    object btnQuant: TAdvGlassButton
      Left = 279
      Top = 23
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Enviar quantidade'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 0
      Version = '1.3.0.2'
      OnClick = btnQuantClick
    end
    object DBEdit3: TDBEdit
      Left = 20
      Top = 35
      Width = 253
      Height = 21
      DataField = 'qtd'
      DataSource = DataSource1
      TabOrder = 1
    end
    object AdvGlassButton5: TAdvGlassButton
      Left = 365
      Top = 23
      Width = 80
      Height = 32
      BackColor = 6052956
      Caption = 'Cancelar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ForeColor = clWhite
      GlowColor = 16760205
      InnerBorderColor = clBlack
      OuterBorderColor = clWhite
      ParentFont = False
      ShineColor = clWhite
      TabOrder = 2
      Version = '1.3.0.2'
      OnClick = AdvGlassButton5Click
    end
  end
  object edtFaltante: TEdit [35]
    Left = 591
    Top = 18
    Width = 129
    Height = 21
    Enabled = False
    TabOrder = 19
  end
  inherited ActionList: TActionList
    Left = 128
    Top = 8
  end
  inherited oCds: TClientDataSet
    ProviderName = 'dspOrdemEnvaseBt'
    RemoteServer = DM.ShCoGen
    object oCdsid: TIntegerField
      FieldName = 'id'
    end
    object oCdscodopenvase: TIntegerField
      FieldName = 'codopenvase'
    end
    object oCdsidprodutos: TIntegerField
      FieldName = 'idprodutos'
    end
    object oCdsqtdprevista: TSingleField
      FieldName = 'qtdprevista'
    end
    object oCdsqtdreal: TSingleField
      FieldName = 'qtdreal'
    end
    object oCdshora: TTimeField
      FieldName = 'hora'
    end
    object oCdsdata: TDateField
      FieldName = 'data'
    end
    object oCdsue: TStringField
      FieldName = 'ue'
      Size = 5
    end
    object oCdslote: TStringField
      FieldName = 'lote'
      Size = 8
    end
    object oCdscpa: TStringField
      FieldName = 'cpa'
      Size = 5
    end
    object oCdsoperador: TStringField
      FieldName = 'operador'
      Size = 30
    end
    object oCdsstatus: TStringField
      FieldName = 'status'
      Size = 15
    end
  end
  object DSProdutos: TDataSource
    DataSet = CdsProdutos
    Left = 40
    Top = 48
  end
  object CdsInsumoEnvase: TClientDataSet
    Aggregates = <>
    AutoCalcFields = False
    Params = <
      item
        DataType = ftWideString
        Name = 'ipdprod'
        ParamType = ptInput
        Value = '0'
      end>
    ProviderName = 'dspTblInsumoEnvase'
    RemoteServer = DM.ShCoGen
    Left = 32
    Top = 288
    object CdsInsumoEnvaseid: TIntegerField
      FieldName = 'id'
    end
    object CdsInsumoEnvasedescricao: TStringField
      FieldName = 'descricao'
      Size = 50
    end
    object CdsInsumoEnvaseinsumosid: TIntegerField
      FieldName = 'insumosid'
    end
    object CdsInsumoEnvaseINSUMODESC: TStringField
      FieldName = 'INSUMODESC'
      Size = 100
    end
  end
  object DSInsumoEnvase: TDataSource
    DataSet = CdsInsumoEnvase
    Left = 72
    Top = 288
  end
  object CdsProdutos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProdutos'
    RemoteServer = DM.ShCoGen
    Left = 8
    Top = 56
    object CdsProdutosid: TIntegerField
      FieldName = 'id'
    end
    object CdsProdutosdescricao: TStringField
      DisplayLabel = 'Selecione o produto...'
      FieldName = 'descricao'
      Size = 50
    end
  end
  object cdsDetalheEnvase: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftWideString
        Name = 'idop'
        ParamType = ptInput
        Value = '0'
      end>
    ProviderName = 'dsTblEnvaseDetalhe'
    RemoteServer = DM.ShCoGen
    OnCalcFields = cdsDetalheEnvaseCalcFields
    Left = 24
    Top = 352
    object cdsDetalheEnvaseid: TIntegerField
      FieldName = 'id'
    end
    object cdsDetalheEnvaseqtd: TSingleField
      FieldName = 'qtd'
    end
    object cdsDetalheEnvaseue: TStringField
      FieldName = 'ue'
      Size = 5
    end
    object cdsDetalheEnvasepreco: TSingleField
      FieldName = 'preco'
    end
    object cdsDetalheEnvasevalortotal: TSingleField
      FieldName = 'valortotal'
    end
    object cdsDetalheEnvasecodproduto: TStringField
      FieldName = 'codproduto'
      Size = 50
    end
    object cdsDetalheEnvaseidordemproducao: TIntegerField
      FieldName = 'idordemproducao'
    end
    object cdsDetalheEnvaseidcodproduto: TIntegerField
      FieldName = 'idcodproduto'
    end
    object cdsDetalheEnvasesQuantidade: TFloatField
      DisplayLabel = 'Qtd.Dispon'#237'vel'
      FieldKind = fkInternalCalc
      FieldName = 'sQuantidade'
    end
    object cdsDetalheEnvasesEstoque: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'sEstoque'
    end
  end
  object dsDetalheEnvase: TDataSource
    DataSet = cdsDetalheEnvase
    Left = 64
    Top = 352
  end
  object cdsQtdProduto: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftWideString
        Name = 'idpr'
        ParamType = ptInput
        Value = '0'
      end>
    ProviderName = 'dspQtdProduto'
    RemoteServer = DM.ShCoGen
    Left = 48
    Top = 416
    object cdsQtdProdutoid: TIntegerField
      FieldName = 'id'
    end
    object cdsQtdProdutodescricao: TStringField
      FieldName = 'descricao'
      Size = 50
    end
    object cdsQtdProdutoinsumosid: TIntegerField
      FieldName = 'insumosid'
    end
    object cdsQtdProdutoINSUMODESC: TStringField
      FieldName = 'INSUMODESC'
      Size = 100
    end
    object cdsQtdProdutoSUM: TFloatField
      FieldName = 'SUM'
    end
    object cdsQtdProdutoidinsumo: TIntegerField
      FieldName = 'idinsumo'
      Required = True
    end
  end
  object cdsEstoqueQuimico: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftWideString
        Name = 'idop'
        ParamType = ptInput
        Value = '0'
      end>
    ProviderName = 'dspTblOrdemProducaoQ'
    RemoteServer = DM.ShCoGen
    OnCalcFields = cdsEstoqueQuimicoCalcFields
    Left = 27
    Top = 164
    object cdsEstoqueQuimicoid: TIntegerField
      FieldName = 'id'
    end
    object cdsEstoqueQuimicoidproduto: TIntegerField
      FieldName = 'idproduto'
    end
    object cdsEstoqueQuimicoidtblordemproducao: TIntegerField
      FieldName = 'idtblordemproducao'
    end
    object cdsEstoqueQuimicopercentual: TSingleField
      FieldName = 'percentual'
      OnChange = cdsEstoqueQuimicopercentualChange
    end
    object cdsEstoqueQuimicoqtd: TSingleField
      FieldName = 'qtd'
    end
    object cdsEstoqueQuimicosDescricao: TStringField
      FieldKind = fkInternalCalc
      FieldName = 'sDescricao'
      Size = 80
    end
  end
  object dsEstoqueQuimico: TDataSource
    DataSet = cdsEstoqueQuimico
    Left = 27
    Top = 220
  end
  object DBQtdProduto: TfrxDBDataset
    UserName = 'QtdProduto'
    CloseDataSource = False
    DataSet = cdsQtdProduto
    BCDToCurrency = False
    Left = 139
    Top = 375
  end
  object DBProduto: TfrxDBDataset
    UserName = 'Produto'
    CloseDataSource = False
    DataSet = CdsProdutos
    BCDToCurrency = False
    Left = 67
    Top = 71
  end
  object DBEstoqueQuimico: TfrxDBDataset
    UserName = 'EstoqueQuimico'
    CloseDataSource = False
    DataSet = cdsEstoqueQuimico
    BCDToCurrency = False
    Left = 59
    Top = 163
  end
  object DBoCds: TfrxDBDataset
    UserName = 'oCDS'
    CloseDataSource = False
    DataSet = oCds
    BCDToCurrency = False
    Left = 99
    Top = 71
  end
  object ReportOP: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 42671.608191388900000000
    ReportOptions.LastChange = 42720.591291076390000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 59
    Top = 220
    Datasets = <
      item
        DataSet = DBDetalheEnvase
        DataSetName = 'DetalheEnvase'
      end
      item
        DataSet = DBEstoqueQuimico
        DataSetName = 'EstoqueQuimico'
      end
      item
        DataSet = DBoCds
        DataSetName = 'oCDS'
      end
      item
        DataSet = DBProduto
        DataSetName = 'Produto'
      end
      item
        DataSet = DBQtdProduto
        DataSetName = 'QtdProduto'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object Shape1: TfrxShapeView
        Left = 11.338590000000000000
        Top = 15.118120000000000000
        Width = 457.323130000000000000
        Height = 147.401670000000000000
        Shape = skRoundRectangle
      end
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 173.858380000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Shape2: TfrxShapeView
          Left = 600.945270000000000000
          Top = 120.944960000000000000
          Width = 434.645950000000000000
          Height = 41.574830000000000000
          Shape = skRoundRectangle
        end
        object Memo1: TfrxMemoView
          Left = 204.094620000000000000
          Top = 34.015770000000000000
          Width = 260.787570000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'Industria e Com'#233'rcio de Cosm'#233'ticos Ltda.')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Top = 170.078850000000000000
          Width = 1058.267716540000000000
          Hint = '0'
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 3.000000000000000000
        end
        object Picture1: TfrxPictureView
          Left = 26.456710000000000000
          Top = 30.236240000000000000
          Width = 173.858380000000000000
          Height = 120.944960000000000000
          Picture.Data = {
            }
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo2: TfrxMemoView
          Left = 204.094620000000000000
          Top = 56.472480000000000000
          Width = 162.519790000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'CNPJ: 63.064968/0001-00')
        end
        object Memo14: TfrxMemoView
          Left = 687.874460000000000000
          Top = 132.283550000000000000
          Width = 272.126160000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Ordem de Produ'#231#227'o - Envase')
          ParentFont = False
        end
      end
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        Height = 173.858380000000000000
        Top = 215.433210000000000000
        Width = 1046.929810000000000000
        object Shape3: TfrxShapeView
          Left = 11.338590000000000000
          Top = 7.559060000000000000
          Width = 1024.252630000000000000
          Height = 147.401670000000000000
          Shape = skRoundRectangle
        end
        object Line2: TfrxLineView
          Left = -7.559060000000000000
          Top = 166.299320000000000000
          Width = 1058.267716540000000000
          Hint = '0'
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 3.000000000000000000
        end
        object Memo3: TfrxMemoView
          Left = 26.456710000000000000
          Top = 18.897650000000000000
          Width = 181.417440000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'C'#243'd. Ordem Produ'#231#227'o:')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          Left = 26.456710000000000000
          Top = 52.913420000000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Produto:')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          Left = 370.393940000000000000
          Top = 86.929190000000000000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Qtd. Prevista:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 370.393940000000000000
          Top = 120.944960000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Qtd. Real:')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 26.456710000000000000
          Top = 120.944960000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Hora:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 26.456710000000000000
          Top = 86.929190000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Data:')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 710.551640000000000000
          Top = 18.897650000000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Lote:')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 710.551640000000000000
          Top = 52.913420000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'CPA:')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 710.551640000000000000
          Top = 86.929190000000000000
          Width = 86.929190000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Operador:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 710.551640000000000000
          Top = 120.944960000000000000
          Width = 60.472480000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Status:')
          ParentFont = False
        end
        object oCDScodopenvase: TfrxMemoView
          Left = 207.874150000000000000
          Top = 18.897650000000000000
          Width = 154.960730000000000000
          Height = 18.897650000000000000
          DataField = 'codopenvase'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."codopenvase"]')
          ParentFont = False
        end
        object Produtodescricao: TfrxMemoView
          Left = 98.267780000000000000
          Top = 52.913420000000000000
          Width = 264.567100000000000000
          Height = 18.897650000000000000
          DataField = 'descricao'
          DataSet = DBProduto
          DataSetName = 'Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[Produto."descricao"]')
          ParentFont = False
        end
        object oCDSqtdprevista: TfrxMemoView
          Left = 491.338900000000000000
          Top = 86.929190000000000000
          Width = 219.212740000000000000
          Height = 18.897650000000000000
          DataField = 'qtdprevista'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."qtdprevista"]')
          ParentFont = False
        end
        object oCDSqtdreal: TfrxMemoView
          Left = 453.543600000000000000
          Top = 120.944960000000000000
          Width = 245.669450000000000000
          Height = 18.897650000000000000
          DataField = 'qtdreal'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."qtdreal"]')
          ParentFont = False
        end
        object oCDSdata: TfrxMemoView
          Left = 75.590600000000000000
          Top = 86.929190000000000000
          Width = 287.244280000000000000
          Height = 18.897650000000000000
          DataField = 'data'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."data"]')
          ParentFont = False
        end
        object oCDShora: TfrxMemoView
          Left = 75.590600000000000000
          Top = 120.944960000000000000
          Width = 287.244280000000000000
          Height = 18.897650000000000000
          DataField = 'hora'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."hora"]')
          ParentFont = False
        end
        object oCDSlote: TfrxMemoView
          Left = 759.685530000000000000
          Top = 18.897650000000000000
          Width = 260.787570000000000000
          Height = 18.897650000000000000
          DataField = 'lote'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."lote"]')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          Left = 755.906000000000000000
          Top = 52.913420000000000000
          Width = 264.567100000000000000
          Height = 18.897650000000000000
          DataField = 'cpa'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."cpa"]')
          ParentFont = False
        end
        object oCDSoperador: TfrxMemoView
          Left = 793.701300000000000000
          Top = 86.929190000000000000
          Width = 226.771800000000000000
          Height = 18.897650000000000000
          DataField = 'operador'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."operador"]')
          ParentFont = False
        end
        object oCDSstatus: TfrxMemoView
          Left = 771.024120000000000000
          Top = 120.944960000000000000
          Width = 249.448980000000000000
          Height = 18.897650000000000000
          DataField = 'status'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[oCDS."status"]')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          Left = 536.693260000000000000
          Top = 120.944960000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          DataField = 'ue'
          DataSet = DBoCds
          DataSetName = 'oCDS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[oCDS."ue"]')
          ParentFont = False
        end
      end
      object Header1: TfrxHeader
        FillType = ftBrush
        Height = 34.015770000000000000
        Top = 449.764070000000000000
        Width = 1046.929810000000000000
        object Memo15: TfrxMemoView
          Left = 11.338590000000000000
          Top = 7.559060000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Produto')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          Left = 370.393940000000000000
          Top = 7.559060000000000000
          Width = 230.551330000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Percentual')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          Left = 604.724800000000000000
          Top = 7.559060000000000000
          Width = 98.267780000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Quantidade')
          ParentFont = False
        end
        object Line3: TfrxLineView
          Top = 30.236240000000000000
          Width = 1058.267716540000000000
          Hint = '0'
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 3.000000000000000000
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 34.015770000000000000
        Top = 506.457020000000000000
        Width = 1046.929810000000000000
        DataSet = DBEstoqueQuimico
        DataSetName = 'EstoqueQuimico'
        RowCount = 0
        object EstoqueQuimicosDescricao: TfrxMemoView
          Left = 11.338590000000000000
          Top = 7.559060000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          DataField = 'sDescricao'
          DataSet = DBEstoqueQuimico
          DataSetName = 'EstoqueQuimico'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[EstoqueQuimico."sDescricao"]')
          ParentFont = False
        end
        object EstoqueQuimicopercentual: TfrxMemoView
          Left = 370.393940000000000000
          Top = 7.559060000000000000
          Width = 230.551330000000000000
          Height = 18.897650000000000000
          DataField = 'percentual'
          DataSet = DBEstoqueQuimico
          DataSetName = 'EstoqueQuimico'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[EstoqueQuimico."percentual"]')
          ParentFont = False
        end
        object EstoqueQuimicoqtd: TfrxMemoView
          Left = 604.724800000000000000
          Top = 7.559060000000000000
          Width = 294.803340000000000000
          Height = 18.897650000000000000
          DataField = 'qtd'
          DataSet = DBEstoqueQuimico
          DataSetName = 'EstoqueQuimico'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[EstoqueQuimico."qtd"]')
          ParentFont = False
        end
      end
      object Header2: TfrxHeader
        FillType = ftBrush
        Height = 30.236240000000000000
        Top = 563.149970000000000000
        Width = 1046.929810000000000000
        object Memo18: TfrxMemoView
          Left = 11.338590000000000000
          Top = 3.779530000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Insumo')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          Left = 449.764070000000000000
          Top = 3.779530000000000000
          Width = 230.551330000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Quantidade')
          ParentFont = False
        end
        object Line4: TfrxLineView
          Top = 26.456710000000000000
          Width = 1058.267716540000000000
          Hint = '0'
          Color = clBlack
          Frame.Typ = [ftTop]
          Frame.Width = 3.000000000000000000
        end
        object Memo21: TfrxMemoView
          Left = 370.393940000000000000
          Top = 3.779530000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'UE')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        Height = 34.015770000000000000
        Top = 616.063390000000000000
        Width = 1046.929810000000000000
        DataSet = DBDetalheEnvase
        DataSetName = 'DetalheEnvase'
        RowCount = 0
        object DetalheEnvaseue: TfrxMemoView
          Left = 366.614410000000000000
          Top = 7.559060000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          DataField = 'ue'
          DataSet = DBDetalheEnvase
          DataSetName = 'DetalheEnvase'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[DetalheEnvase."ue"]')
          ParentFont = False
        end
        object DetalheEnvasecodproduto: TfrxMemoView
          Left = 11.338590000000000000
          Top = 7.559060000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          DataField = 'codproduto'
          DataSet = DBDetalheEnvase
          DataSetName = 'DetalheEnvase'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[DetalheEnvase."codproduto"]')
          ParentFont = False
        end
        object DetalheEnvaseqtd: TfrxMemoView
          Left = 445.984540000000000000
          Top = 7.559060000000000000
          Width = 234.330860000000000000
          Height = 18.897650000000000000
          DataField = 'qtd'
          DataSet = DBDetalheEnvase
          DataSetName = 'DetalheEnvase'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[DetalheEnvase."qtd"]')
          ParentFont = False
        end
      end
    end
  end
  object DBDetalheEnvase: TfrxDBDataset
    UserName = 'DetalheEnvase'
    CloseDataSource = False
    DataSet = cdsDetalheEnvase
    BCDToCurrency = False
    Left = 259
    Top = 375
  end
  object cdsControleOp: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspControleOp'
    RemoteServer = DM.ShCoGen
    Left = 264
    Top = 48
    object cdsControleOpid: TIntegerField
      FieldName = 'id'
    end
    object cdsControleOpidop: TIntegerField
      FieldName = 'idop'
    end
    object cdsControleOpqtd: TSingleField
      FieldName = 'qtd'
    end
    object cdsControleOpdatains: TDateField
      FieldName = 'datains'
    end
    object cdsControleOphorains: TTimeField
      FieldName = 'horains'
    end
  end
  object dsControleOp: TDataSource
    Left = 303
    Top = 47
  end
  object DataSource1: TDataSource
    DataSet = cdsControleOp
    Left = 360
    Top = 304
  end
end
